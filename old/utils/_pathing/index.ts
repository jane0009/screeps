...
import { MATRIX_CACHE_TIME, MATRIX_LAYER_CONTAINERS, MATRIX_LAYER_OFF_ROADS, MATRIX_LAYER_PREFER_CONTAINERS, MATRIX_LAYER_ROADS, MATRIX_LAYER_STRUCTURES, MATRIX_LAYER_SWAMP_ROADS, MATRIX_LAYER_TERRAIN, MATRIX_LAYER_TUNNELS, TERRAIN_COST } from '../constants';
...
class PathingManager {
  ...
  constructor(options: AnyStringOptions = {}) {
    this.onRoomEnter = options.onRoomEnter;
    this.getCreepWorkingTarget = options.getCreepWorkingTarget;
    this.getCreepInstance = options.getCreepInstance || ((creep: Creep) => creep);
    this.getCreepEntity = options.getCreepEntity || ((instance: any) => instance);
  ...
  clearMatrixCacheRoom(roomName: string) {
    this.matrixCache.delete(roomName);
  }


  moveTo(creep: Creep, target: any, defaultOptions: AnyStringOptions = {}) {
    ...
    if (!fatigue) {
      ...
      if (newPath) {
        if (
          ...
          let costCallback;
          if (prevCostCallback) {
            costCallback = (roomName: string, matrix: any) => {
  ...
  moveOffRoad(creep: Creep, options: AnyStringOptions = {}) {
    const instance = this.getCreepInstance(creep);
  ...
  * getAdjacentPositions(pos) {
  ...
  // cost matrix
  getCostMatrix(roomName: string, options: AnyStringOptions = {}, matrixKey: number | undefined = undefined) {
  ...
  addHostilesToMatrix(matrix: any, pos: RoomPosition, room: Room, range: number) {
    Utils.lookInRange(pos, room, LOOK_CREEPS, range).forEach(item => {
      const creep = item.creep;
      if ((creep instanceof Creep) && !creep.my) {
        const { x, y } = creep.pos;
        matrix.setFast(x, y, 255);
      }
    });
  ...
  makeStructuresMatrix(room: Room, options: AnyStringOptions) {
    const {
    ...
    if (
      ...
      if (!ignoreStructures) {
        room.find(FIND_MY_CONSTRUCTION_SITES).forEach(construction => {
          if (!(construction instanceof StructureContainer || construction instanceof StructureRoad || construction instanceof StructureRampart)) {
...
if (!Creep.prototype.originalMoveTo) {
  ...
  Creep.prototype.moveTo = function (target, defaultOptions: AnyStringOptions = {}) {
    if (this.spawning) {
      return 0;
    }
    const options = {
